...
class MarkovAutoregression(markov_regression.MarkovRegression):
    ...
    def __init__(self, endog, k_regimes, order, trend='c', exog=None,
                 ...
                 dates=None, freq=None, missing='none', exog_other=None):
        ...
        # Add lagged values of exog_other to exog if specified
        if exog_other is not None:
            exog_other_lags = lagmat(exog_other, 2)
            if exog is not None:
                exog = np.column_stack([exog, exog_other_lags])
            else:
                exog = exog_other_lags
